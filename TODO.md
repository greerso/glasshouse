...
- [x] **2026-09-16: WCEC email SENT** (issue #2 step 4) to chad.gray@williamsoncounty-tn.gov — Gmail thread `1a0aaadefe35334a`, text as archived in `docs/research/2026-11-election/wcec-email-draft-2026-09-16.md`. Asks for the list certified Sep 11, where/when it will be posted, and the Nov 3 sample ballot + early-voting schedule. Sent through the Composio `gmail` toolkit; the claude.ai Gmail MCP session was expired again (second time), and the stale Sep 8 draft no longer exists in either mailbox. #2
- [ ] **Watch for Chad Gray's reply** (phone fallback (615) 790-5711 if nothing by ~Sep 23). Save the reply into `docs/research/2026-11-election/raw/`, write a new `candidates-observed-*.md`, then copy names into `glasshouse-web/src/content/elections/thompsons-station-2026-11.json` and rebuild. Names go up only from an official WCEC source. #2
